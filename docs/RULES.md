...
> 규칙 목록: R0 용어 · R1 선지 · R2 메뉴스키마 · R3 학습 · R4 재료/장바구니 · R5 인분 · R6 Claude호출 · R7 컨벤션 · R8 날짜·저장·인증 · R9 시드풀 · **R10 식단 공유** · **R11 메뉴 검색/추가**.
...
## R3. 학습 규칙 (취향 프로파일)
저장소: **`src/lib/preferences.ts`** — localStorage(`prefs:v1`)가 단일 출처(source of truth),
로그인 시 `preference_profiles`에 **best-effort 미러**(UI를 막지 않음). 필드: `cuisineWeights`,
`tagWeights`, `dislikedMenuIds`, `dislikedIngredients`, `allergies`.
...
| 메뉴 선택(확정) | `select` | 해당 cuisine/tags **+3** | ⏳ 미연동 |
...
- ⚠️ localStorage는 `dislikedMenuIds`(id 기반), `shared/schema.ts`의 `PreferenceProfileSchema`는 `dislikedMenuNames` — 미러 시 둘을 구분. 추후 통일 검토.
...
## R5. 인분 수 / 수량 스케일  ✅ 구현
- **고정 기본 인분 수는 없다.** 사용자가 설정에서 인분 수를 선택한다. 저장 키 **`mp.servings`**(`preferences.ts`의 `getServings/useServings/setServings`).
...
6. **두 가지 모드** (`generate-menus` Edge Function):
   ...
   - **`mode:'single'`**: `requestedName`+`meal`로 **단일 메뉴 1개** 생성(R11 "추가 생성"용). R2 단일 객체로 검증, 1회 재시도. 단일 모드는 `ANTHROPIC_API_KEY`만 필요(캐시/서비스롤 불필요).
...
- 폴더:
  ...
  meal-planner/
    ...
    supabase/
      ...
      migrations/    0001_init.sql, 0002_sharing.sql
...
6. **장바구니 범위**: 활성 식단(R10)의 확정 엔트리 재료를 R4대로 합산. 체크 상태 유지(현재 localStorage `cart-checks:v1`).
...
## R11. 메뉴 검색 / 추가 (＋버튼)
슬롯 선지 끝 **＋버튼** → `MenuSearchModal`. 원하는 메뉴를 직접 찾거나 새로 만든다.

1. **시드 검색 우선**: 입력에 따라 `seedPool`을 퍼지 검색(이름/태그/설명 부분일치 + 서브시퀀스, 점수순 상위). 고르면 그 슬롯에 확정(`selectMenu`). **AI/배포 없이 동작.**
